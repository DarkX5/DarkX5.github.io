...
m.f.isNullOrEmpty = function (value, fallbackValue) {
	var isNull = false;
	var emptyArr = ["undefined", "null", "NaN", ""];
	var emptyObj = ["{}", "[]"];
	if (emptyObj.indexOf(JSON.stringify(value)) > -1) {
		if (m.f.isNullOrEmpty(value.style))
			isNull = true;
	}
	try {
...
m.f.isEmpty = function (value) {
	var emptyArr = ["NaN", ""];
	var emptyObj = ["{}", "[]"];
	if (emptyObj.indexOf(JSON.stringify(value)) > -1)
		if (m.f.isEmpty(value.style))
			return true;
...
m.f.parseBool = function (value) {
	/* negative numbers return false */
	if (!m.f.isNullOrEmpty(value)) {
		switch (Object.prototype.toString.call(value)) {
			...
			case "[object Array]":
				...
				for (var i = 0; i < value.length; i += 1) {
					resultArr.push(m.f.parseBool(value[i]));
				}
				return resultArr;
				break;
...
m.f.toggleVisibility = function (elements, hideClass, visibilityValue) {
	debugger;
	if (!m.f.isNull(elements)) {
		var obj = elements;
		...
		if (objType === "[object Array]" || objType === "[object HTMLCollection]")
			...
			for (var i = 0; i < elements.length; i += 1)
				obj.push(m.f.isNullOrEmpty(elements[i], elements));
		}

		if (m.f.isNullOrEmpty(hideClass)) {
			if (!m.f.isNullOrEmpty(visibilityValue)) {
			...
			if (!m.f.isNullOrEmpty(visibilityValue)) {
				...
				if (visibility === "") {
					for (var i = 0; i < obj.length; i += 1)
						obj[i].className = m.f.replaceAll(obj[i].className, " " + hideClass, "");
				} else {
					for (var i = 0; i < obj.length; i += 1)
...
m.f.replaceAll = function (target, search, replace) {
	if (m.f.isNullOrEmpty(target)) {
		console.log("replaceAll error: target is null or empty");
		return null;
	}
	if (m.f.isNullOrEmpty(replace))
		replace = "";
...
m.f.getLS = function (varName) {
	if (m.f.isNullOrEmpty(varName)) {
	...
	if (m.f.isNullOrEmpty(returnVar))
		returnVar = "{}";
...
m.f.setLS = function (varName, newVal) {
	...
	if (m.f.isNullOrEmpty(newVal))
		newVal = {};

	localStorage.setItem(varName, JSON.stringify(newVal));
}

m.f.downloadFile = function (fileName, fileData, fileType) {
	if (m.f.isNullOrEmpty(fileName)) {
	...
	var fType = m.f.isNullOrEmpty(fileType) ? "text/plain" : fileType;
	fName = fName[0];
...
/* v INIT MASTER VARS v */

	/* init page vars */
	m.OS = m.f.getOSName();
	m.path = "";
	if (m.OS == 'Android')
		m.path = "/sdcard/0Sync/Work/HTML/Tasker Webviews";
	else
		m.path = ".";
